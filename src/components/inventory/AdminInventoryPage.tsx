...
import { ADMIN_ROLE_KEY, ROLE_PERMISSIONS, type AdminRole } from '../../lib/constants';
...
function conditionBadge(item: InventoryItem) {
  const ratio = item.quantity_total > 0 ? item.quantity_damaged / item.quantity_total : 0;
...
export default function AdminInventoryPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      msgApi.error(err instanceof Error ? err.message : 'Gagal menghapus item.');
    } finally {
  ...
  const stats = useMemo(() => {
    return items.reduce(
      ...
      { itemCount: 0, totalUnits: 0, goodUnits: 0, damagedUnits: 0, inUseUnits: 0, availableUnits: 0 },
    );
  }, [items]);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        service_date: values.service_date ? values.service_date.format('YYYY-MM-DD') : null,
        warranty_date: values.warranty_date ? values.warranty_date.format('YYYY-MM-DD') : null,
      };
  ...
  const columns: TableColumnsType<InventoryItem> = [
    ...
    },
    {
      ...
      render: (name: string, row) => (
        <Space direction="vertical" size={2}>
          ...
          <Space size={6}>
            <Tag color={CATEGORY_COLORS[row.category] ?? 'default'} style={{ fontSize: 11, margin: 0 }}>
              {row.category}
            </Tag>
            {row.brand && (
              <Text type="secondary" style={{ fontSize: 11 }}>{row.brand}</Text>
            )}
            {row.model && (
              <Text type="secondary" style={{ fontSize: 11 }}>{row.model}</Text>
            )}
          </Space>
        </Space>
      ),
    ...
    },
    {
      ...
      width: 140,
      render: (v: string | null) =>
        v ? <Tag>{v}</Tag> : <Text type="secondary" style={{ fontSize: 12 }}>—</Text>,
    },
    {
      title: 'Total',
    ...
    },
    {
      ...
      width: 64,
      align: 'center' as const,
      render: (v: number) => <Text strong style={{ color: '#16a34a' }}>{v}</Text>,
    },
    {
      title: 'Rusak',
      ...
      align: 'center' as const,
      render: (v: number) =>
        v > 0 ? <Text strong style={{ color: '#dc2626' }}>{v}</Text> : <Text type="secondary">0</Text>,
    },
    {
      title: 'Digunakan',
      ...
      align: 'center' as const,
      render: (v: number) =>
        v > 0 ? <Text strong style={{ color: '#d97706' }}>{v}</Text> : <Text type="secondary">0</Text>,
    },
    {
      title: 'Tersedia',
      ...
      align: 'center' as const,
      render: (v: number) =>
        v > 0 ? <Text strong style={{ color: '#2563eb' }}>{v}</Text> : <Text type="secondary">0</Text>,
    },
    {
      title: 'Foto',
      key: 'photo',
          ...
          <Tooltip title="Lihat foto">
            <a href={row.photo_url} target="_blank" rel="noopener noreferrer">
              <img
                ...
                width={32}
                height={32}
                style={{ objectFit: 'cover', borderRadius: 4, display: 'block', margin: '0 auto' }}
              />
            </a>
          </Tooltip>
        ) : (
    ...
    },
    {
      ...
      width: 110,
      render: (_: unknown, row: InventoryItem) => {
        if (!row.warranty_date) return <Text type="secondary" style={{ fontSize: 12 }}>—</Text>;
        const daysLeft = dayjs(row.warranty_date).diff(dayjs(), 'day');
        if (daysLeft < 0) return <Tag color="default" style={{ fontSize: 11 }}>Kadaluarsa</Tag>;
        ...
        if (daysLeft <= 30) return (
          <Tooltip title={`${daysLeft} hari lagi`}>
            <Tag color="warning" style={{ fontSize: 11 }}>{row.warranty_date}</Tag>
          </Tooltip>
        );
        return <Tag color="success" style={{ fontSize: 11 }}>{row.warranty_date}</Tag>;
      },
    },
    {
      title: 'Kondisi',
    ...
    <AdminLayout activeKey="inventory">
      ...
      <Space direction="vertical" size={20} style={{ width: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          ...
          {canWrite && (
            <Button type="primary" icon={<PlusOutlined />} size="large" onClick={openAdd} block={typeof window !== 'undefined' && window.innerWidth < 768}>
              Tambah Item
            </Button>
          )}
        </div>
        ...
        <Row gutter={[12, 12]}>
          <Col xs={12} sm={8} lg={4}>
            <Card size="small" style={{ borderRadius: 10, textAlign: 'center' }}>
              <Statistic title="Jenis Item" value={stats.itemCount} valueStyle={{ fontSize: 'clamp(16px, 3.5vw, 20px)' }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} lg={4}>
            <Card size="small" style={{ borderRadius: 10, textAlign: 'center' }}>
              <Statistic title="Total Unit" value={stats.totalUnits} valueStyle={{ fontSize: 'clamp(16px, 3.5vw, 20px)' }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} lg={4}>
            <Card size="small" style={{ borderRadius: 10, textAlign: 'center' }}>
              <Statistic title="Kondisi Baik" value={stats.goodUnits} valueStyle={{ color: '#16a34a', fontSize: 'clamp(16px, 3.5vw, 20px)' }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} lg={4}>
            <Card size="small" style={{ borderRadius: 10, textAlign: 'center' }}>
              <Statistic title="Rusak" value={stats.damagedUnits} valueStyle={{ color: '#dc2626', fontSize: 'clamp(16px, 3.5vw, 20px)' }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} lg={4}>
            <Card size="small" style={{ borderRadius: 10, textAlign: 'center' }}>
              <Statistic title="Sedang Digunakan" value={stats.inUseUnits} valueStyle={{ color: '#d97706', fontSize: 'clamp(16px, 3.5vw, 20px)' }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} lg={4}>
            <Card size="small" style={{ borderRadius: 10, textAlign: 'center' }}>
              <Statistic title="Tersedia" value={stats.availableUnits} valueStyle={{ color: '#2563eb', fontSize: 'clamp(16px, 3.5vw, 20px)' }} />
            </Card>
          </Col>
        </Row>
        ...
        <Card style={{ borderRadius: 12 }} styles={{ body: { padding: 0 } }}>
          ...
          <Table<InventoryItem>
            ...
            pagination={{ pageSize: 20, showSizeChanger: false, showTotal: (t) => `${t} item` }}
          />
        </Card>
      </Space>
      ...
      {/* ── Add / Edit Modal ──────────────────────────────────────────── */}
      <Modal
        title={editingItem ? `Edit: ${editingItem.name}` : 'Tambah Item Inventaris'}
        open={modalOpen}
